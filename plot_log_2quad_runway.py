...
def plot_data(data):
    ...
    plt.subplot(512)
    plt.plot(t,data[:,4],  '--b', alpha=0.7, label='Quad1_X')
    plt.plot(t,data[:,5], '--r', alpha=0.7, label='Quad1_Y')
    plt.plot(t,data[:,8], '--g', alpha=0.7, label='Quad2_X')
    plt.plot(t,data[:,9],  'b', alpha=0.7, label='Quad2_Y')
#    plt.plot(t,data[:,6],  'r', alpha=0.7, label='Y')
#    plt.plot(t,data[:,7],  'g', alpha=0.7, label='Z')
    plt.grid();plt.legend()

    plt.subplot(513)
    plt.plot(t,data[:,6], alpha=0.7, label='Quad1_Vx')
    plt.plot(t,data[:,7], alpha=0.7, label='Quad1_Vy')
    plt.plot(t,data[:,10], alpha=0.7, label='Quad2_Vx')
    plt.plot(t,data[:,11], alpha=0.7, label='Quad2_Vy')
#    plt.plot(t,data[:,7], alpha=0.7, label='Vz')
    ...
    plt.subplot(514)
    plt.plot(data[:,12:21],label='Quad1_past_actions', alpha=0.3)
    plt.grid()#;plt.legend()
